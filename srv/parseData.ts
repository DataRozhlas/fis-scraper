...
// sort files
fileNames.filter(name => name.includes("CC.html")).sort();
//parse the files
...
//       .childNodes[3] as HTMLElement).getAttribute("title") || undefined;
...
const file = await Bun.file(`../data/2022-12-CC.html`).text();
const root = parse(file);
const calendarData = root.querySelector("#calendardata.tbody");
const rows = calendarData?.childNodes.filter((node) =>
    node.nodeType === 1
